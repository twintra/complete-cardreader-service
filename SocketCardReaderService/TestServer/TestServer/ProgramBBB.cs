...
class ProgramA
{
    ...
    private ThaiIDCard card = new ThaiIDCard();
    ...
    public void Start()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                while (listClient.Count != 0)
                {
                    listClient[listClient.Count - 1].Close();
                }
                ...
                Console.WriteLine("==> Client Connected: {0}", listClient.Count);
                ...
                while (true)
                {
                    ...
                    while (!stream.DataAvailable)
                    {
                        ...
                        if (is_exp <= 0)
                        {
                            ...
                            onDisconnect(stream, client, listClient);
                            disconnected = true;
                            break;
                        }
                    };
    ...
    public void onDisconnect(NetworkStream stream, TcpClient client, List<TcpClient> listClient)
    {
        stopCardReader(stream);
    ...
    public string createJsonString(string message, string keyname = "message")
    {
        Dictionary<string, string> json = new Dictionary<string, string>();
        json.Add(keyname, message);
        return DictionaryToJson(json);
    }
    public void doCardReader(NetworkStream stream)
    {
        try
        {
            ...
            if (readers.Length != 0)
            {
                ...
                {
                    this.card.MonitorStart(readers[i]);
                }
            }
            this.card.eventCardInsertedWithPhoto += (e) =>
            {
    ...
    public void stopCardReader(NetworkStream stream)
    {
        try
        {
            string[] readers = this.card.GetReaders();
            Console.WriteLine("==> readers connected : {0}", readers);
            if (readers.Length != 0)
            {
                for (int i = 0; i < readers.Length; i++)
                {
                    this.card.MonitorStop(readers[i]);
                }
            }
